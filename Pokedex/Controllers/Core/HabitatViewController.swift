...
class HabitatViewController: UIViewController {
    ...
    var pokemonEntries: [PokemonSpecy] = []
    var selectedHabitat: Habitat = .cave
    ...
    lazy var tagCollectionView: TTGTextTagCollectionView = {
        ...
        let tags = Habitat.allCases.map({ TTGTextTag(content: TTGTextTagStringContent(
    ...
    private func fetchData() {
        APICaller.shared.apiCall(url: API.habitat(selectedHabitat).apiUrl) {[weak self] (result: HabitatResponse) in
            guard let self = self else { return }
            self.pokemonEntries = result.pokemonSpecies
...
extension HabitatViewController: TTGTextTagCollectionViewDelegate {
    
    func textTagCollectionView(_ textTagCollectionView: TTGTextTagCollectionView!, didTap tag: TTGTextTag!, at index: UInt) {
        let description = tag.content.getAttributedString().string
        selectedHabitat = Habitat(rawValue: description.lowercased())  ?? .cave
        fetchData()
...
extension HabitatViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configureCellByName(pokemonSpecy: pokemonEntries[indexPath.item])
        return cell
    }
